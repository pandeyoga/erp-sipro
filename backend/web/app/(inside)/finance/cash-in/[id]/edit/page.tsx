...
export default function EditConstractionPage() {
  ...
  const handleDelete = async () => {
    if (!selected) return;
    try {
      await axios.delete(`/finance/cash-in/transaction/${selected.transaction_id}`);
      handleReload()
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-gray-600 mb-6">Update Cash In</h1>
      <CashInForm id={id} mode="edit" handleReload={handleReload}/>
      <DataTable
        ref={TableRef}
        endpoint={`/finance/cash-in/${id}/transaction`}
        columns={columns}
        actions={(row) => (
