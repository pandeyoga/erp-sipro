...
class AuthService
{
    ...
    public function login(array $credentials): array
    {
        ...
        return [
            'success' => true,
            'data' => $this->respondWithToken($token, $user->formattedPermissions()),
        ];
    }
     ...
     */
     ...
     public function refresh(): array
     {
         try {
            ...
            if (User::where('email', $user->email)->first()->is_active == 0) {
                return [
                        'success' => false,
                        'message' => 'Account is disabled',
                    ];
            ...
            return [
                'success' => true,
                'data' => $this->respondWithToken(JWTAuth::parseToken()->refresh()),
            ];
     ...
     * @return array
     */
    protected function respondWithToken(string $token, $permissions): array
    {
        return [
            'access_token' => $token,
            'token_type' => 'bearer',
