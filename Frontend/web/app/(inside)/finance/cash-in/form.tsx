...
export default function CashInForm({ mode, id, handleReload }: Props) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (mode === "edit" && id) {
        await axios.put(`/finance/cash-in/${id}`, data);
        toast.success("Cash In berhasil diperbarui");
      } else {
        await axios.post("/finance/cash-in", data);
        toast.success("Cash In berhasil ditambahkan");
      }

      router.push("/finance/cash-in");
    } catch (err: any) {
  ...
  const [typeSubCategories, setTypeSubCategories] = useState<OptionData>();
  useEffect(() => {
    if (sub_category_id) {
      axiosInstance
        .get(`/finance/cash-in/sub-sub-categories/${sub_category_id}`)
        .then((response) => {
          setTypeSubCategories(response.data.data);
        });
    }
  }, [sub_category_id]);
      ...
      try {
        if (project_id) {
          const res = await axios.get(
            `/property/siteplan/${project_id}/list-option-property`
          );
  ...
  useEffect(() => {
    ...
    const penjualan_rumah: any = categories.find(
      (value) => value.name == "Penjualan Rumah"
    );
    if (penjualan_rumah) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <fieldset className="border border-base-300 rounded-xl p-6 space-y-1">
        <legend className="text-lg font-semibold px-2">Cash Info</legend>
        ...
        <div className="form-control">
          ...
          {mode == "create" || !watch("bank_account") ? (
            <select
              className="select select-bordered w-full"
      ...
      {mode == "edit" && formInfo && (
        <PaymentInfoParent
          ...
          bank_list={bank_list}
          bank_account_id_default={watch("bank_account_id")}
        />
      )}
    </form>
  );
...
function PaymentInfo({
  ...
  }, [child_form.paid_amount,bank_account_id_default]);

  useEffect(()=>{
    setValue("bank_account_id",bank_account_id_default)
  },[])
  ...
  const onSubmit = async () => {
    ...
    try {
      setLoading(true);
      await axios.put(`/finance/cash-in/${cash_in_id}/transaction`, {
    ...
    <div className="grid grid-cols-6 gap-4 border-1 rounded-lg p-4 border-gray-200">
      <div className="grid grid-cols-1 md:grid-cols-3 col-span-5 gap-4">
        ...
        <div className="form-control">
          <label className="label">
            <span className="label-text capitalize">
              Remaining Invoice {child_form.name}
            </span>
          </label>
          <NumericFormat
            thousandSeparator="."
